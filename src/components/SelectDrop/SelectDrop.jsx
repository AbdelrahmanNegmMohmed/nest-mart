    ...
    <ClickAwayListener onClickAway={() => setisopenSelect(false)}>
      <div className="celectDropWrapper posttion-relative ">
        ...
        <span className="openSelect" onClick={openSelect}>
          {selectItems.length > 14
            ? selectItems.substr(0, 14) + "...."
            : selectItems}{" "}
        ...
        {isopenSelect === true && (
          <div className="selectDrop">
            ...
            <ul className="searchResults">
              <li
                key={0}
                onClick={() => closSelect(0, placeholder)}
                className={`${slectedIndex === 0 ? "active" : ""}`}
              >
                {placeholder}
              </li>
              ...
              {listDta.map((item, index) => {
                return (
                  <li
                    key={index + 1}
                    onClick={() => closSelect(index + 1, item)}
                    className={`${slectedIndex === index + 1 ? "active" : ""}`}
                  >
                    {item}
                  </li>
                );
